...
| 81 | **Save-compat (bug GRAVE)** : `saveGame` ne whitelistait pas `playerName`/`calling`/`seed`/`traits` → recharger réinitialisait le nom, re-tirait la vocation au hasard, changeait la graine, vidait les traits. Whitelist complétée + garde `tests/persistence.test.ts` | _(ce commit)_ |
