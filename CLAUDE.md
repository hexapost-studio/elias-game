...
| 82 | **Tour guidé : dernière étape avalée** (trouvé en revue de code) : `useTargetRect` ne réinitialisait pas la mesure au changement de cible → la cascade de saut des étapes absentes (`.event-card`/`#choices-area`, idle) débordait sur `.btn-age`. État clé sur `selector` + garde `tests/tutorialOverlay.test.tsx` | _(ce commit)_ |
